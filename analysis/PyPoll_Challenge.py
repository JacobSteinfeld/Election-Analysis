...
with open(file_to_save, "w") as txt_file:
    ...
    for county in county_votes:
        ...
        votes_per_county = float(votes) / float(total_votes) * 100
        
         # 6d: Print the county results to the terminal.
        results_county = (f"{county}: {votes_per_county:.1f}% ({votes:,})\n")
        print(results_county)
         # 6e: Save the county votes to a text file.
        txt_file.write(results_county)
         # 6f: Write an if statement to determine the winning county and get its vote count.
        if (votes > county_winning_count) and (votes_per_county > county_winning_percentage):
            county_winning_count = votes
            highest_county = county
            county_winning_percentage = votes_per_county
